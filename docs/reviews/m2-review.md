...
<section id="m2-overview" class="review-fragment" data-analysis-status="final">
<h2>M2.0 — Compatibility inventory</h2>
<p><strong>结论（Interpretation）：</strong>在目前 pinned runtime 上，Automatic Prefix Caching/FP8 KV cache/online FP8 weight 与 Qwen3 draft speculative configuration 均有成功的 canonical candidate；Qwen2.5 draft speculative configuration 在 startup 阶段因为 0.5B 与 7B 的 vocab size 不一致导致失败，于是切换到 Qwen3-0.6B/8B。这里的 supported 只表示对应配置在本环境可启动并完成所列请求，不证明性能、质量或跨版本兼容性。</p>
<table>
...
<p><strong>Observed Fact：</strong>4个 supported candidate 的公开 summary 均记录 success 与 raw validation passed；Qwen2.5 failure evidence 记录 0 measured request 与 startup failure。APC、量化、512-token speculative 的 A/B 和 4k/8k/16k long-context run set 是 active showcase 范围。</p>
...
<section id="m2-prefix-cache" class="review-fragment" data-analysis-status="final">
<h2>M2.1 — Prefix-cache canonical comparison</h2>
<p><strong>结论（Interpretation）：</strong>在固定 prefix-heavy workload 的 C1/C16 canonical A/B 中，run-shared candidate 的 token hit/prefill/TTFT 与 waiting 读数共同支持“prefix reuse 减少 measured prefill 工作”的结论。C16 确认了 bounded concurrency 下的机制收益，但不是 capacity，<code>C_eff</code> 或全局最优点。</p>
...
<p><strong>Observed Fact 1：</strong>两侧均在新 runtime 上完成 C1/C16 × 3 repetitions；各 6/6 cases 完整、96/96 requests 成功且 raw validation passed。对应请求的 input tokens 全部匹配，public summaries 保留 query/hit counter delta 与 derived hit ratio。</p>
<p><strong>Observed Fact 2：</strong>C1 的 E2E p95 median 为 4.291→4.961 秒，没有观测到明显的 E2E 收益。96 对请求中有 17 对 output token 数不同，因此 throughput/TPOT/E2E 的影响因子应小于 counter/prefill/TTFT。</p>
<h3>范围与执行缺口</h3>
<ul><li>Priming-inclusive 成本尚未物化，公开投影也不保留 warm-up evidence，因此当前不发布成本结论。部分记录与重算分析可见 <a href="../../docs/reviews/m2-priming-metrics.md">Priming Metrics 重算记录</a> </li><li>性能 run 不保存输出文本；结果不能用于输出质量结论，也不能外推到其他 prompt 或实际 hit-rate 分布。</li></ul>
</section>

<section id="m2-quantization" class="review-fragment" data-analysis-status="final">
<h2>M2.2 — Quantization canonical comparisons</h2>
<p><strong>结论（Interpretation）：</strong>在当前 pinned runtime 与 C1 workload 下，FP8 KV 将 runtime 报告的 KV capacity 提高约 92.7%，但冻结质量集出现负向 flip。所以不能视为透明替代。Online FP8 weight 的 decode 指标改善，4-case 质量集未观测到 flip。Online FP8 的参数实际为 FP8 weight/<code>activation=None</code>/BF16 compute/KV auto，不是已验证的 W8A8。两组结果均不外推为全局性能或质量结论。</p>
...
<p><strong>Observed Fact：</strong>四条 canonical run 均使用 fresh runtime，完成 C1 × 3 repetitions；每条 24/24 measured requests 成功 + raw validation passed + lifecycle graceful。4 份公开 output-evaluation summary 均记录 4/4 cases + 0 request error，且 item id 与 scorer 对齐；FP8 KV 的 <code>long-01</code> 为 <code>true→false</code>，<code>format-01</code> 在四侧的 strict <code>json_exact</code> 都为 false。</p>
<p><strong>公开证据边界：</strong>Tier A 仅保留 output-evaluation summary，没有原始 evaluation response 或 frozen case file。公开页面可以复核 score/flip，不能从公开 projection 重算质量 summary 或独立复核输出文本细节。目前没有预声明质量的阈值，这里只报告变化，不判定“可接受”。</p>
<h3>范围与执行缺口</h3>
<ul><li>性能采集只有 C1 canonical，未执行额外 bounded exploratory 点；目前不确定并发收益/capacity 点/全局最优。</li><li>KV pair 的 measured output tokens 为 1,200 → 1,483，E2E 与 request/output throughput 不能作完全独立的 A/B 归因；TTFT、TPOT 只作为该有界请求集的观测。</li><li>质量结论仅覆盖固定 4 cases；约 1K 的 retrieval case 不代表 8K long-context accuracy。</li></ul>
</section>

<section id="m2-speculative" class="review-fragment" data-analysis-status="final">
<h2>M2.3 — Speculative decoding canonical comparison</h2>
<p><strong>结论（Interpretation）：</strong>在当前 pinned runtime 中，Qwen3-0.6B draft + Qwen3-8B target 不是通用的单节点 latency 优化：512-token canonical 在<b>降低</b> TPOT/E2E + 提高 output TPS的同时<b>增加</b> 了TTFT。结果支持一个以目标函数划分的适用边界——tested completion/decode-sensitive workload 受益，TTFT-primary 交互请求为负收益；tested canonical range 未出现整体 E2E/TPS negative point。</p>
...
<p><strong>Observed Fact：</strong>两侧均完成了 C1/C16 × 3 repetitions；各 96/96 requests 成功且 raw validation passed。所有 measured requests 均为 33 input / 512 output tokens + <code>finish_reason=length</code>。Candidate 在每个 concurrency 内三次 counter delta 相同，acceptance 派生值已进入 summary。</p>
<h3>范围与执行缺口</h3>
...
<section id="m2-long-context" class="review-fragment" data-analysis-status="final">
...
<h3>范围与 Unknown</h3>
<ul><li>最高验证的是 14,912 input-token 请求成功；<code>max_model_len=32768</code> 配置与 Tier B startup check 不代表 32k 的请求能力。每点只有一次 + 4 个顺序请求。这个测试只做通路，不追求 tail latency + capacity 或严格线性。</li><li>4k 的 output 为 57 tokens，8k/16k 为 55，因此 E2E 只作配套观测；结论以 actual input/TTFT/prefill 与 KV usage 为主。</li><li>GPU framebuffer telemetry 为 unsupported，只能报告现有代理指标未显示压力。三条 server log 在 measured 请求完成后的 SIGTERM shutdown 阶段均有 cleanup traceback 和 <code>EngineDeadError</code>；wrapper 仍记录 exit 0 + 无 OOM/restart + lifecycle success。</li></ul>
</section>
