# High-Level Roadmap (v1 — Archived)

> **已归档，不是执行计划。** 当前执行计划见 [`Roadmap.md`](Roadmap.md)（v2）。
>
> 本文件保留 v1 的十 Milestone 全量设计，用途有两个：
>
> 1. **M0 / M1 的原始范围与 Exit Criteria** —— 这两个 Milestone 已完成，其定义以本文件为准；
> 2. **长期 backlog** —— v2 删除或降级的内容（Memory Supervisor CRD、自写 Scheduler Plugin、
>    LLM Autoscaler、Multi-Runtime Benchmark、Production Simulation 九场景矩阵、跨节点 TP）
>    在这里保留完整设计，8 周窗口之后可按需取回。
>
> v1 → v2 的逐项变更理由见 `Roadmap.md` 附录 A。
...
在脱离 Kubernetes 的条件下理解 vLLM 的核心执行路径，建立单节点、单运行时、可重复的 benchmark 管线，并形成 workload-specific operating references 与简洁的 tested capacity/compatibility bounds。
...
benchmarks/reports/benchmark-summary-viewer.html
showcase/m1/
```
...
2. Per-workload bounded selection：用预声明 criteria 辅助解释 `C1 / C_eff / C_pressure`，不执行统一大矩阵，也不由性能指标自动停止 sweep。
3. Short-input / short-output。
4. Short-input / long-output。
5. Long-input / short-output。
6. Long-input / long-output。
7. `max_model_len`、`max_num_seqs`、memory utilization 最小 OVAT。
8. Small / Medium 模型代表性 compatibility 和 scale check。
9. 统一内存使用量随模型、上下文和并发变化的曲线。


Representative baseline/medium model: Qwen2.5-0.5B-Instruct/Qwen2.5-7B-Instruct

- Dense decoder-only instruction model
- Same model family as the small baseline
- Approximately 3B–10B parameter class
- Preferably at least one order of magnitude larger than baseline
- Single-node, tensor-parallel-size=1
- Same resolved precision as baseline
- No quantization, CPU offload, speculative decoding, or custom long-context path
- Supported or reproducibly unsupported by the pinned ARM64 vLLM runtime

### Exit Criteria

- [x] Offline 与 online serving 均可通过脚本复现。
- [x] Streaming client 能记录 TTFT 和 token timestamp。
- [x] 每组实验区分 warm-up 与 measured runs。
- [x] 所有失败、timeout、OOM 和 server restart 均被保留。
- [x] 已获得稳定的 TTFT、E2E 与 token throughput；ITL/TPOT 和内存只在已验证的支持范围内报告。
- [ ] 各 workload 已建立 operating references 或 bounded Unknown，并完成至少一个简洁的 capacity/compatibility bound test。
- [x] 能解释 Prefill-heavy 与 Decode-heavy workload 的差异。
- [x] 结论基于原始数据，而非单次观察。

### 可选扩展

- Prefix Cache optimization / hit-vs-miss comparison
- Chunked Prefill
...
| M1 Single-Node vLLM Baseline | In progress | M1.4 per-workload benchmark and interactive closeout |
...
- [x] 单节点 vLLM serving baseline 已建立并保留原始请求级数据。
