...
| C | `long-short` | >2,000 tokens | 32 | prefill and TTFT |
| D | `long-long` | >2,000 tokens | 512 | prefill, decode, and KV-cache pressure |

The source prompts are deterministic, but token counts depend on the model tokenizer. Treat the intended ranges as hypotheses until the response `usage.prompt_tokens` confirms them. If a short case exceeds its range, shorten only that case, document the change, and rerun all repetitions for that case. If a long case is below 2,000 tokens, increase its repeated context and document the new source revision.
