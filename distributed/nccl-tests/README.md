...
## Repository-Specific M0 Note

This directory vendors upstream nccl-tests and adds local M0 launch helpers:

- `scripts/nccl-m0-test-all-reduce.sh`;
- `scripts/nccl-m0-test-all-gather.sh`;
- `scripts/verify-mpi.sh`.

The reviewed M0 result is in [`docs/environment/nccl-baseline.md`](../../docs/environment/nccl-baseline.md). Private raw evidence is under `artifacts/m0-private/20260805-m0-final/distributed/` and must not be published directly.

The final bundles retain the fully expanded `mpirun` invocations in
`command.txt`. Their contexts record `git_dirty=true`, so the exact bytes of
locally built binaries are not reconstructed solely by the commit; this is a
non-blocking provenance caveat, not a limitation on the observed command,
correctness result, transport logs, or raw counter deltas.

The remainder of this file is the upstream nccl-tests documentation.
